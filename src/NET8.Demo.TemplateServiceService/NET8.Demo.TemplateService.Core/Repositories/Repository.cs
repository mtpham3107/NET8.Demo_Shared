...
namespace NET8.Demo.TemplateService.Core.Repositories;
...
public class Repository<TEntity> : IRepository<TEntity> where TEntity : EntityBase
{
    ...
    private readonly TemplateServiceDbContext _context;

    public Repository(TemplateServiceDbContext context, ILogger<Repository<TEntity>> logger, IStringLocalizer<Repository<TEntity>> localizer)
    {
    ...
    public async ValueTask SoftDeleteAsync(Guid? userId, Guid id, params Expression<Func<TEntity, object>>[] includes)
    {
        try
        {
            var entity = await FilterQuery(x => x.Id == id, null, includes).FirstOrDefaultAsync() ?? throw new EntityNotFoundException(_localizer["Error.DataNotFound"]).WithData("id", id);
